...
const DataProvider = ({ children }) => {
  ...
  const localUrl = "http://192.168.50.173:8000/decibel-meter/";
  const herokuUrl =
    "https://decibel-meter-react-native.herokuapp.com/decibel-meter/";
  const dbDiff = 35;
  ...
  const checkAndCompareDecibelByTime = () => {
    ...
    if (
      ...
      if (
        lastMin.avg > currentDecibelDate.avg ||
        lastMin.avg < currentDecibelDate.avg
      ) {
      ...
      if (
        ...
        if (!currentDecibelDate.alarm) {
          ...
          if (
            ...
            setError(
              `${currentDecibelDate.date} High volume detected:${decibel.currentDecibelNum} `
            );
          }
          if (
            decibel.currentDecibelNum < currentDecibelDate.avg - dbDiff ||
            ...
            setError(
              `${currentDecibelDate.date} Low volume detected:${decibel.currentDecibelNum} `
            );
          }
        }
      }
  ...
  const createAudioPermissionAndRecord = async () => {
    try {
      ...
      volumeCallback = () => {
        ...
        if (
          decibel.decibelNumHistoryArr.length % currentSpec == 0 &&
          user.decibelHistory.length * 240 < loop
        ) {
  ...
  const decibelHistoryBtn = async () => {
    const userFromDb = await createDecibelHistory(
      user.username,
      user.password,
      lastMin.daily
    );
  ...
  const start = () => {
    if (volumeCallback !== null && volumeInterval === null)
      volumeInterval = setInterval(volumeCallback, 250);
    setIsStart(true);
  };
  const stop = () => {
  ...
  const createDecibelHistory = async (name, password, arr) => {
    ...
    const response = await axios.put(`${herokuUrl}user-page`, userToFetch);

    return response.data;
  };
  const selectedUser = async (createdUser) => {
    const response = await axios.post(`${herokuUrl}sign-in`, createdUser);

    return response.data;
  };

  const addUser = async (addedUser) => {
    const response = await axios.post(`${herokuUrl}sign-up`, addedUser);
    return response.data;
  };

  // const getSensors = async (data) => {
  //   const response = await axios.post(`${localUrl}user-page`, data);
  // };
  ...
  const _subscribe = () => {
    setSubscription2(
      Accelerometer.addListener(setData2)
    );
  };
